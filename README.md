...
cd ai-rbac-lecture
pnpm install
cp .env.example .env       # optional — paste an OpenAI key, or skip for fake mode
pnpm dev                   # → http://localhost:5173
```

The app boots immediately. A **fake-mode router** dispatches tools by keyword when no API key is set — so you can follow the entire lecture offline.

---

## What you'll write

Six numbered spots in two files. Search the codebase for `LIVE-CODE` to find them:

- `src/auth/roles.ts` — Permission union, ROLE_PERMISSIONS matrix, hasPermission
- `src/auth/can.ts` — the ABAC layer (`can()` function)
- `src/auth/tools.ts` — two tool definitions
...
- **Boss Hoxha** (admin) — sees everything, but cannot delete other people's invoices
- **Carla / Marko** (accountant) — see all invoices including drafts, can approve
